...
namespace SOWApi.Controllers
{
    ...
    [ApiController]
    public class SOWRequirementTechnologyController : ControllerBase
    {
        private readonly ISOWRequirementTechnologyService _Service;
        private readonly ILogger<SOWRequirementTechnologyController> _logger;

        public SOWRequirementTechnologyController(ISOWRequirementTechnologyService Service, ILogger<SOWRequirementTechnologyController> logger)
        {
            _Service = Service;
            _logger = logger;
        }
        ...
        public async Task<ActionResult<IEnumerable<SOWRequirementTechnologyDTO>>> GetAll()
        {
        ...
        [HttpGet("{id}")]
        [Authorize(Roles = "Admin, Director, Project Manager, Team Lead, Team Member")]
        public async Task<ActionResult<SOWRequirementTechnologyDTO>> Get(string id)
        {
        ...
        public async Task<ActionResult<SOWRequirementTechnologyDTO>> Add([FromBody] SOWRequirementTechnologyDTO _object)
        {
        ...
        [HttpPut("{id}")]
        [Authorize(Roles = "Admin, Director, Project Manager")]
        public async Task<IActionResult> Update(string id, [FromBody] SOWRequirementTechnologyDTO _object)
        {
            if (!ModelState.IsValid)
            {
